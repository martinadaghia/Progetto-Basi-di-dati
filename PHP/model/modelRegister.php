...
<body class="text-center">
  ...
  <img class="mb-4" src="risorse/immagini/logo.png" alt="" height="100px">
    ...
    <form name="formRegister" action="" method="post" enctype="multipart/form-data"> <!-- post per maggiore sicurezza, per passare dati sensibili alla pagina --> 
  ...
  if(isset($_POST["submit"])){
    ...
    && !empty($_POST["dataNascitaRegister"])  && !empty($_POST["luogoNascitaRegister"])){
      ...
      if($tipo=="amministratore"){
        ...
        $immagine=file_get_contents($_FILES["fotoRegister"]["tmp_name"]);
        $cv=file_get_contents($_FILES["cvRegister"]["tmp_name"]);
        $dipartimento=$_POST["dipartimentoRegister"];

        $nomeFoto = basename($_FILES["fotoRegister"]["name"]);
        $nomeCv = basename($_FILES["cvRegister"]["name"]);
        $dirFoto=__DIR__."/risorse/immagini";
        $dirCv=__DIR__."/risorse/file";
        
        move_uploaded_file($_FILES["fotoRegister"]["tmp_name"], $dirFoto."/".$nomeFoto); // spostare file dentro xamp
        move_uploaded_file($_FILES["cvRegister"]["tmp_name"], $dirCv."/".$nomeCv);

        $dbh->caricaFoto($username, $immagine);
        $dbh->caricaCV($username, $cv);

        $ultimaFoto=$dbh->getUltimaFoto();
        $ultimoCv=$dbh->getUltimoCv();

        $dbh->insertSpeaker($username, $nome, $ultimoCv[0]["idFile"], $ultimaFoto[0]["idLogo"], $dipartimento);
        ...
        $nomeFoto = $_FILES["fotoRegister"]["name"];
        $nomeCv = $_FILES["cvRegister"]["name"];
        $dirFoto="/Applications/XAMPP/xamppfiles/htdocs/ProgettoBasi/PHP/model/risorse/immagini";
        $dirCv="/Applications/XAMPP/xamppfiles/htdocs/ProgettoBasi/PHP/model/risorse/file";

        move_uploaded_file($_FILES["fotoRegister"]["tmp_name"], $dirFoto."/".$nomeFoto); // spostare file dentro xamp
        move_uploaded_file($_FILES["cvRegister"]["tmp_name"], $dirCv."/".$nomeCv);

        $dbh->caricaFoto($username."_".$nomeFoto, $immagine);
        $dbh->caricaCV($username."_".$nomeCv, $cv);

        $ultimaFoto=$dbh->getUltimaFoto();
        $ultimoCv=$dbh->getUltimoCv();


        $dbh->insertPresenter($username, $nome, $ultimoCv[0]["idFile"], $ultimaFoto[0]["idLogo"], $dipartimento);
      }
