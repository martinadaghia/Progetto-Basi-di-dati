...
<?php
$infoUtente = $dbh->getInfoUtente($_SESSION["username"]);
?>

<div class="m-5" style ="text-align: left">
  <b><p class="col-md fs-5">INFORMAZIONI UTENTE</p></b>
  <p>User: <b><?php echo($infoUtente[0]["Username"]); ?></b></p>
  <p>Nome: <b><?php echo($infoUtente[0]["Nome"]); ?></b></p>
  <p>Cognome: <b><?php echo($infoUtente[0]["Cognome"]); ?></b></p>
  <p>Data di Nascita: <b><?php echo($infoUtente[0]["DataNascita"]); ?></b></p>
  <p>Luogo di Nascita: <b><?php echo($infoUtente[0]["LuogoNascita"]); ?></b></p>

  <?php
  if($_SESSION["tipo"] == "speaker") {
    $info = $dbh->getInfoSpeaker($_SESSION["username"]);
  ?>

  <p>CV: <b><?php echo($info[0]["Cv"]); ?></b></p>
  <p>Nome Universita: <b><?php echo($info[0]["Nome"]); ?></b></p>
  <p>Locazione Dipartimento: <b><?php echo($info[0]["Dipartimento"]); ?></b></p>
  <p>Tipo Utente: <b>Speaker</b></p>

  <?php
    } else if($_SESSION["tipo"] == "presenter") {
      $info = $dbh->getInfoPresenter($_SESSION["username"]);
  ?>

  <p>CV: <b><?php echo($info[0]["Cv"]); ?></b></p>
  <p>Nome Universita: <b><?php echo($info[0]["Nome"]); ?></b></p>
  <p>Locazione Dipartimento: <b><?php echo($info[0]["Dipartimento"]); ?></b></p>
  <p>Tipo Utente: <b>Presenter</b></p>

  <?php
    } else if($_SESSION["tipo"] == "ammministratore") {
      echo("<p>Tipo Utente: <b>Amministratore</b></p>"); 
    } else {
      echo("<p>Tipo Utente: <b>Normale</b></p>");
    }
  ?>
...
?>
<div class="m-5" style ="text-align: left">
  <b><p class="col-md fs-5">FOTO PROFILO</p></b>

  <?php $articoli=$dbh->getFotoProfiloSpeaker($_SESSION["username"]); ?>
  <?php echo("<img src='.".DIRECTORY_SEPARATOR."risorse".DIRECTORY_SEPARATOR."immagini".DIRECTORY_SEPARATOR.$articoli[0]["NomeFile"]."' width='300' height='200'"); ?>     
...
<div class="m-5" style ="text-align: left">
  ...
  <?php $articoli=$dbh->getFotoProfiloPresenter($_SESSION["username"]); ?>
  <?php echo("<img src='.".DIRECTORY_SEPARATOR."risorse".DIRECTORY_SEPARATOR."immagini".DIRECTORY_SEPARATOR.$articoli[0]["NomeFile"]."' width='300' height='200'"); ?>     
...
?>
  ...
  <div class="m-5" style ="text-align: left">
    ...
    if(isset($_POST["inserisciArticoli"])){
      ...
      $nomeFile = basename($_FILES["filePresentazione"]["name"]);

      $dirFoto=__DIR__."".DIRECTORY_SEPARATOR."risorse".DIRECTORY_SEPARATOR."file";

      move_uploaded_file($_FILES["filePresentazione"]["tmp_name"], $dirFoto.DIRECTORY_SEPARATOR.$_SESSION["username"]."_".$nomeFile); // spostare file dentro xamp

      $dbh->caricaFileEsterni($_SESSION["username"]."_".$nomeFile, $immagine);
      $ultimaFile=$dbh->getUltimoFile();
      
      $dbh->inserisciArticoli($oraInizio, $oraFine, $numSequenza, $titoloArticolo, $numPagine, $ultimaFile[0]["idFile"], $codiceSessione);
    }
    ?>
...
<?php
  ...
  if(isset($_POST["modificaFotoPresenter"])){
    $nomeFoto = basename($_FILES["fotoPresenter"]["name"]);
    $dirFoto=__DIR__.DIRECTORY_SEPARATOR."risorse".DIRECTORY_SEPARATOR."immagini";

    move_uploaded_file($_FILES["fotoPresenter"]["tmp_name"], $dirFoto.DIRECTORY_SEPARATOR.$_SESSION["username"]."_".$nomeFoto); // spostare file dentro xamp

    $dbh->modificaFotoPresenter($_SESSION["username"], $_SESSION["username"]."_".$nomeFoto);
  }
  ...
  if(isset($_POST["modificaFotoSpeaker"])){
    $nomeFoto = basename($_FILES["fotoSpeaker"]["name"]);
    $dirFoto=__DIR__.DIRECTORY_SEPARATOR."risorse".DIRECTORY_SEPARATOR."immagini";

    move_uploaded_file($_FILES["fotoSpeaker"]["tmp_name"], $dirFoto.DIRECTORY_SEPARATOR.$_SESSION["username"]."_".$nomeFoto); // spostare file dentro xamp
  
    $dbh->modificaFotoSpeaker($_SESSION["username"], $_SESSION["username"]."_".$nomeFoto);
  }
