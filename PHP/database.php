<?php
    ...
    class DatabaseHelper{
        ...
        public function caricaCv($nomeFile, $datiCv){
            $query="call inserimentoFileEsterni(?,?,@a)";
            $stmt=$this->db->prepare($query); 
            $stmt->bind_param('ss', $nomeFile, $datiCv);
            $stmt->execute();
        }
        ...
        public function getUltimoCv(){
            $query="SELECT idFile FROM FILEESTERNI ORDER BY idFile desc limit 1"; 
            $stmt=$this->db->prepare($query); 
            $stmt->execute();
            $result=$stmt->get_result();
            return $result->fetch_all(MYSQLI_ASSOC); 
