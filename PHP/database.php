<?php
    ...
    class DatabaseHelper{
        ...
        public function getVerificaRegistrazioneConf(){
            $query="SELECT Acronimo, Nome, AnnoEdizione FROM CONFERENZA as C 
                    LEFT JOIN REGISTRAZIONE as R ON R.AcronimoConferenza=C.Acronimo AND R.AnnoEdizioneConferenza=C.AnnoEdizione 
                    WHERE NOT(R.UsernameUtente<=>'lucia') AND C.CampoSvolgimento='ATTIVA' GROUP BY AcronimoConferenza"; 
            $stmt=$this->db->prepare($query); 
            $stmt->execute();
            $result=$stmt->get_result();
            return $result->fetch_all(MYSQLI_ASSOC); 
