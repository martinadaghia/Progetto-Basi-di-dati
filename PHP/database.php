<?php
    ...
    class DatabaseHelper{
        ...
        public function getVerificaRegistrazioneConf(){
            $query="SELECT Acronimo, Nome, AnnoEdizione FROM REGISTRAZIONE as R 
                    LEFT JOIN CONFERENZA as C ON R.AcronimoConferenza=C.Acronimo AND R.AnnoEdizioneConferenza=C.AnnoEdizione 
                    WHERE NOT(R.UsernameUtente<=>'".$_SESSION['username']."') GROUP BY AcronimoConferenza"; 
            $stmt=$this->db->prepare($query); 
            $stmt->execute();
            $result=$stmt->get_result();
            return $result->fetch_all(MYSQLI_ASSOC); 
