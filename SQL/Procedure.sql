...
CREATE PROCEDURE creazioneArticoli(IN InputOraInizio TIME, IN InputOraFine TIME, IN InputNumSequenza INT, IN InputTitolo VARCHAR(50), IN InputNumPagine INT, IN InputFilePresentazione INT, 
    IN InputUsernameUtentePresenter VARCHAR (100), IN InputCodiceSessione INT, OUT Esito VARCHAR(100)) 
BEGIN
	...
	IF numArticoli = 0 THEN
		INSERT INTO PRESENTAZIONEDIARTICOLI (OraInizio, OraFine, NumSequenza, Titolo, NumPagine, FilePresentazione, StatoSvolgimento, UsernameUtentePresenter, CodiceSessione) 
			VALUES (InputOraInizio, InputOraFine, InputNumSequenza, InputTitolo, InputNumPagine, InputFilePresentazione, 'Non coperto', InputUsernameUtentePresenter, InputCodiceSessione);
