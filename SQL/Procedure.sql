...
DELIMITER |
CREATE PROCEDURE registrazioneSpeaker(IN InputUsername VARCHAR(100), IN InputNome VARCHAR(100), IN InputCv VARCHAR(30), IN InputFoto INT, IN InputDipartimento VARCHAR(50), OUT Esito VARCHAR(100)) 
BEGIN
...
DELIMITER |
CREATE PROCEDURE registrazionePresenter(IN InputUsername VARCHAR(100), IN InputNome VARCHAR(100), IN InputCv VARCHAR(30), IN InputFoto INT, IN InputDipartimento VARCHAR(50), OUT Esito VARCHAR(100)) 
BEGIN
...
DELIMITER |
CREATE PROCEDURE inserimentoFileEsterni(IN InputNomeFile VARCHAR(50), IN InputDatiFile LONGBLOB, OUT Esito VARCHAR(100)) 
BEGIN
...
DELIMITER |
CREATE PROCEDURE modificaFotoSpeaker(IN InputUsernameUtente VARCHAR(100), IN InputNomeFoto VARCHAR(50), OUT Esito VARCHAR(100)) 
BEGIN
	UPDATE logo AS l JOIN speaker AS s ON s.Foto=l.idLogo
	SET NomeFile = InputNomeFoto WHERE UsernameUtente = InputUsernameUtente;
END;
|
DELIMITER ;
...
DELIMITER |
CREATE PROCEDURE modificaFotoPresenter(IN InputUsernameUtente VARCHAR(100), IN InputNomeFoto VARCHAR(50), OUT Esito VARCHAR(100)) 
BEGIN
	UPDATE logo AS l JOIN presenter AS p ON p.Foto=l.idLogo
	SET NomeFile = InputNomeFoto WHERE UsernameUtente = InputUsernameUtente;
END;
|
DELIMITER ;
...
BEGIN
	DECLARE dt DATETIME;
    SET dt = NOW();
	INSERT INTO MESSAGGIO (Testo, DataMessaggio, CodiceChat, UsernameUtente) VALUES (InputTesto, dt, InputCodiceChat, InputUsernameUtente);
	SET Esito = 'Inserito';
END
|
DELIMITER ;
...
DELIMITER |
CREATE PROCEDURE creazioneConferenza(IN InputAcronimo VARCHAR(10), IN InputAnnoEdizione YEAR, IN InputNome VARCHAR(50), IN InputLogo INT, IN InputTotaleSponsorizzazioni DOUBLE, IN InputUsernameUtente VARCHAR (100), IN InputGiorno DATE, OUT Esito VARCHAR(100)) 
BEGIN
	...
	IF numConferenza = 0 THEN
		...
		INSERT INTO PROGRAMMAGIORNALIERO (DataConferenza, AcronimoConferenza, AnnoEdizioneConferenza) VALUES (InputGiorno, InputAcronimo, InputAnnoEdizione); /* dalla data di oggi estrae l'anno */
...
DELIMITER |
CREATE PROCEDURE creazioneSessione(IN InputTitolo VARCHAR(50), IN InputNumeroPresentazioni INT, IN InputOraInizio TIME, IN InputOraFine TIME, IN InputLink VARCHAR(200), IN InputCodiceProgrammaGiornaliero VARCHAR (10), OUT Esito VARCHAR(100)) 
BEGIN
	DECLARE numSessione INT;
	DECLARE codSessione INT;
	SET numSessione = (SELECT COUNT(*) FROM SESSIONE WHERE (Titolo = InputTitolo AND OraInizio = InputOraInizio AND OraFine = InputOraFine));
	IF numSessione = 0 THEN
		INSERT INTO SESSIONE (Titolo, NumeroPresentazioni, OraInizio, OraFine, Link, CodiceProgrammaGiornaliero) VALUES (InputTitolo, InputNumeroPresentazioni, InputOraInizio, InputOraFine, InputLink, InputCodiceProgrammaGiornaliero);
        SET codSessione = (SELECT Codice FROM SESSIONE ORDER BY Codice desc limit 1);
		INSERT INTO CHAT (CodiceSessione) VALUES (codSessione);
...
DELIMITER |
CREATE PROCEDURE creazioneTutorial(IN InputOraInizio TIME, IN InputOraFine TIME, IN InputNumSequenza INT, IN InputTitolo VARCHAR(50), IN InputAbstract VARCHAR(500), IN InputCodiceSessione INT, OUT Esito VARCHAR(100)) 
BEGIN
	DECLARE numTutorial INT;
	SET numTutorial = (SELECT COUNT(*) FROM TUTORIAL WHERE (Titolo = InputTitolo AND OraInizio = InputOraInizio AND OraFine = InputOraFine));
	IF numTutorial = 0 THEN
		INSERT INTO TUTORIAL (OraInizio, OraFine, NumSequenza, Titolo, Abstract, CodiceSessione) VALUES (InputOraInizio, InputOraFine, InputNumSequenza, InputTitolo, InputAbstract, InputCodiceSessione);
...
DELIMITER |
CREATE PROCEDURE creazioneArticoli(IN InputOraInizio TIME, IN InputOraFine TIME, IN InputNumSequenza INT, IN InputTitolo VARCHAR(50), IN InputNumPagine INT, IN InputFilePresentazione INT, 
IN InputCodiceSessione INT, OUT Esito VARCHAR(100)) 
BEGIN
	DECLARE numArticoli INT;
	SET numArticoli = (SELECT COUNT(*) FROM PRESENTAZIONEDIARTICOLI WHERE (Titolo = InputTitolo AND OraInizio = InputOraInizio AND OraFine = InputOraFine));
	IF numArticoli = 0 THEN
		INSERT INTO PRESENTAZIONEDIARTICOLI (OraInizio, OraFine, NumSequenza, Titolo, NumPagine, FilePresentazione, StatoSvolgimento, CodiceSessione) 
			VALUES (InputOraInizio, InputOraFine, InputNumSequenza, InputTitolo, InputNumPagine, InputFilePresentazione, 'Non coperto', InputCodiceSessione);
